    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            if (stp === "list") {
                ...
                let number = 8
                let page = 1
                let a = trackl.length / number

                const generateEmbed = async (start) => {
                    let sayı = page === 1 ? 1 : page * number - number + 1
                    const current = trackl.slice(start, start + number)
                ...
                const canFitOnOnePage = trackl.length <= number
                                ...
                                components: [
                                    new ActionRowBuilder({
                                        components: [
                                            ...
                                            deleteButton,
                                            ...(currentIndex + number < trackl.length ? [forwardButton] : []),
                                        ],
                                    }),
                                ],
                            }).catch(e => {
            ...
            if (stp === "lists") {
                const playlist = await db?.playlist?.findOne({userID: interaction.user.id}).catch(() => {
                })
