        ...
        try {
            ...
            interaction.reply({embeds: [embed], components: [buttons, buttons2, buttons3]}).then(async Message => {
                ...
                col.on('collect', async (button) => {
                    ...
                    let filter = button.customId
                    if (!filter) return interaction.editReply({content: lang.msg29, ephemeral: true}).catch(e => {
                    })
                    ...
                    if (filters.includes(filter.toLowerCase())) {
                        if (queue.filters.has(filter)) {
                            ...
                            embed.setDescription(lang.msg31.replace("{filter}", filter).replace("{status}", "❌"))
                        ...
                        const filter = filters.find((x) => x.toLowerCase() === filter.toLowerCase())
