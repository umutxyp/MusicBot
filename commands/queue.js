...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            let count = 8
            let page = 1
            let a = trackl.length / count

            const generateEmbed = async (start) => {
                let sayı = page === 1 ? 1 : page * count - count + 1
                const current = trackl.slice(start, start + count)
    ...
    ${current.map(data =>
            ...
            const canFitOnOnePage = trackl.length <= count
                            ...
                            components: [
                                new ActionRowBuilder({
                                    components: [
                                        ...
                                        deleteButton,
                                        ...(currentIndex + count < trackl.length ? [forwardButton] : []),
                                    ],
                                }),
                            ],
                        }).catch(e => {
